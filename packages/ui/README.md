...
- **Forms & inputs** — text inputs with validation, date pickers, switches, checkbox variants, file upload
...
`gsap`, `motion`, `three`, `canvas-confetti` and `rough-notation` come with the package — the animation-heavy components depend on them directly.
