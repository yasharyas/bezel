...
|---|---|---|
| 1. The file is the interface | 90 of 91 files are standalone | `MultiStepLoader` (excluded from the CLI) |
...
| 4. Contrast is a gate | all tokens, and every measurable pair the 91 previews render: 587 at the last run, 0 failing | 65 rendered pairs sit on images, gradients, canvases or blend modes and are reported, not measured; components still carry pre-token literals |
...
`CardGrid`, and then a pass that took the library to 89, and PixelAvatar made it 90, then ConfettiFirecracker 91. They have not
